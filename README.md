...
NASQ is a performance-first, multipurpose Salla Twilight theme. Its default preset targets abaya and modest-fashion stores, while its settings and component library are designed to feel familiar to merchants and implementers coming from mature WordPress commerce themes.

- Mobile-first storefront and product pages.
- Grouped settings with practical presets instead of unrelated switches.
- Multiple header, product-card, archive and product-page styles.
- A large native section library, starting with Shop the Look and Occasion Collections.
- No unnecessary runtime dependency for visual effects.

See [WOODMART-MAPPING.md](WOODMART-MAPPING.md) for the feature parity map and Salla platform boundaries.

> Status: version `0.1.0` is the architecture foundation, not a marketplace release candidate yet. Publishing requires a verified Salla Partners account, a demo store, real catalog testing, screenshots, support details and Salla review.
